...
import { resetLimiter } from "../middlewares/rate-limiter.middleware";

const router = Router();

// @route   GET /api/system/reset
// @desc    Reset database (clear all tables)
// @access  Public but protected by secret key and rate limiting
router.get("/reset", resetLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
